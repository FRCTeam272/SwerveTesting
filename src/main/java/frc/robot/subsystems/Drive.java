...
public class Drive extends SubsystemBase {
  ...
  private ShuffleboardTab driveTab = Shuffleboard.getTab("DriveTab");
	...
	private Drive() {
		...
		frontLeft = new swerveModule(Constants.FL_TurnChannel, Constants.FL_SpeedChannel, invertDrive, invertSteer);

		backLeft = new swerveModule(Constants.BL_TurnChannel, Constants.BL_SpeedChannel, invertDrive, invertSteer);

		frontRight = new swerveModule(Constants.FR_TurnChannel, Constants.FR_SpeedChannel, invertDrive, invertSteer);

		backRight = new swerveModule(Constants.BR_TurnChannel, Constants.BR_SpeedChannel, invertDrive, invertSteer);
  ...
  public void processInput(double forward, double strafe, double omega, boolean deadStick) {
    ...
    // position/calibration of drives.
		double angleFL = angle(B, D) + Constants.FL_Offset + lfOffset;
		double angleBL = angle(A, D) + Constants.BL_Offset + lbOffset;
		double angleFR = angle(B, C) + Constants.FR_Offset + rfOffset;
		double angleBR = angle(A, C) + Constants.BR_Offset + rbOffset;
